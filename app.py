...
mode = st.radio("Select mode", options=['Sender', 'Reciever'], horizontal=True)
...
if mode == 'Sender':
    ...
    with input_col:
        with st.container(border=True, height=650):
            ...
            if grid_selection_mode == "User defined":
                ...
                st.info(f"Select {grid_type.lower()} zones by clicking the grid.")
            ...
            if st.session_state.selected_hazard_grids or st.session_state.selected_safety_grids:
                st.write("Copy code below and paste Recipient Tab")
    ...
    with input_col:
        with st.container(border=True, height=650):
            st.code(st.session_state.city, language="python",)

            sms_payload = st.text_area("Paste SMS Code", value=None, height=90)
            ...
            if st.session_state.user_location is not None:
                ...
                distance_metric.metric(label="DISTANCE", value="N/A", border=True)
                time_metric.metric(label="EST. TIME", value="N/A", border=True)
            ...
            affected_area_metric.metric(label="AFFECTED AREA", value=f"{round(grids_area_sqkm, 2)} ha", border=True)
